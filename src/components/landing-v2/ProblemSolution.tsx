    ...
    <div className="container mx-auto px-4 grid lg:grid-cols-2 gap-16 items-center">
      <div className="space-y-4">
        {problems.map((problem, i) => (
          <motion.div 
            key={problem.text}
            className="p-4 rounded-lg border border-glass-border bg-navy-light/30 flex items-center gap-4 shadow-lg"
            initial={{ opacity: 0, x: -50 }}
            whileInView={{ opacity: 1, x: 0 }}
            viewport={{ once: true, amount: 0.5 }}
            transition={{ duration: 0.5, delay: i * 0.1 }}
          >
            <div className="w-10 h-10 bg-navy-lighter rounded-md flex items-center justify-center">
              <problem.icon className="w-5 h-5 text-amber-400" />
            </div>
            <p className="font-medium text-slate-300">{problem.text}</p>
          </motion.div>
        ))}
      </div>
      <motion.div
